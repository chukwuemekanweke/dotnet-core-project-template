...
namespace BackendProjectTemplate.WebAPI.IntegrationTests.Authentication.Passwords;

[Collection(nameof(ContainersCollection))]
public sealed class When_ChangingPassword_WithValidCurrentPassword_Should(ContainersFixture fixture)
    : WebApiIntegrationTestBase(fixture), IAsyncLifetime
{
    private const string CurrentPassword = "P@ssw0rd123!";
    private const string NewPassword = "N3wP@ssword!";
    ...
    public async Task InitializeAsync()
    {
        ...
        await AuthenticateAsync(CurrentPassword);
    }
    ...
    [Fact]
    public async Task AcceptNewPasswordForSignIn()
    {
        await WhenChangingPassword();
        await ThenTheNewPasswordCanBeUsedToSignIn();

        async Task WhenChangingPassword()
        {
            _response = await Client.PutAsJsonAsync(
                EndpointUrl.Passwords.V1,
                new ChangePasswordRequest(CurrentPassword, NewPassword, NewPassword));
        }

        async Task ThenTheNewPasswordCanBeUsedToSignIn()
        {
            ...
            using var signInResponse = await Client.PostAsJsonAsync(
                ...
                new SignInRequest(_email, NewPassword));

            signInResponse.StatusCode.ShouldBe(HttpStatusCode.OK);
        }
    }

    private async Task AuthenticateAsync(string password)
    {
    ...
    private async Task CreateVerifiedUserAsync()
    {
        ...
        (await identityService.CreateAsync(user, CurrentPassword)).Succeeded.ShouldBeTrue();
