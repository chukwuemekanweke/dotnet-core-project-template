...
public sealed class IdentityEndpointsTests(CustomWebApplicationFactory factory) : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task SignUp_ReturnsAccepted_AndStoresOtp()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/identity/sign-up", new SignUpRequest
        {
            Email = email,
    ...
    public async Task VerifyOtp_ActivatesTheAccount()
    {
        ...
        await client.PostAsJsonAsync("/api/identity/sign-up", new SignUpRequest
        {
            Email = email,
            Password = "P@ssw0rd123!",
            FirstName = "Grace",
        ...
        var verifyResponse = await client.PostAsJsonAsync("/api/identity/sign-up/otp", new SignUpOtpRequest
        {
            Email = email,
    ...
    public async Task SignIn_ReturnsToken_AfterVerification()
    {
        ...
        await client.PostAsJsonAsync("/api/identity/sign-up/otp", new SignUpOtpRequest
        {
            Email = email,
            Otp = factory.OtpDeliveryService.GetCode(email)!
        });

        var signInResponse = await client.PostAsJsonAsync("/api/identity/sign-in", new SignInRequest
        {
            Email = email,
            Password = password
        });
